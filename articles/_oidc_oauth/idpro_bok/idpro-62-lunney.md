...
元記事：https://bok.idpro.org/article/id/62/
...
The short answer to this question is: it depends. Sometimes there are limitations as to what SPs can do, as well as IdPs. There are pros and cons to both integrations, so it really is just a matter of choice (or limitation) between the IdPs and SPs. [^9]

The IDPro Body of Knowledge article “ [Introduction to Identity - Part 2: Access Management](https://bok.idpro.org/article/id/45/) ” by Pamela Dingle offers an interesting view of the evolution of authentication and access control tools. [^10] In particular, the section ‘Mobile & API Innovation Gave Us OAuth & Delegated Authorization Frameworks’ offers some interesting insights into the evolution that led to the development of OAuth despite the existence of SAML.
...
ID="HpiyLr\_zVMK.jxdUHXxRvjJ8Fwy" IssueInstant="2020-11-24T01:53:06.809Z" Version="2.0"
...
\--url “${sso\_prefix}/authorization?response\_type=code&redirect\_uri=${redirect\_uri}&scope="openid profile”&client\_id=${client\_id}
...
\--header 'Authorization: Basic base64(urlencode("${client\_id}:${client\_secret}))' \\
